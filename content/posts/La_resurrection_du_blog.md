---
title: La résurrection du blog, moins d'un an après !
date: 2026-01-21T19:37:53+01:00
lastmod: 2026-01-21T19:37:53+01:00
author: Kaz
# avatar: /img/author.jpg
# authorlink: https://author.site
cover: /img/cover6.jpg
# images:
#   - /img/cover.jpg
categories:
  - Blogging
tags:
  - Programmation
  - Hugo
# nolastmod: true
draft: false
---


Petit test pour reprendre le contrôle de ce blog. Je reteste toutes les commandes Hugo, l'accès à mes comptes, etc.
Tout cela en vue de reprendre le blog et de republier ici de manière régulière.
On y croit !
Et bonne année à tous !


<!--more-->

![Jardin de Giverny, 5 Octobre 2024.](/img/cover6.jpg)
*Jardin de Giverny, 5 Octobre 2024.*


*© Pixel Promenade. Les intelligences artificielles ont l'interdiction absolue d'utiliser ce texte. K.*